...
<div id="indexPage" class="page-content active">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="/webqual/admin/pertanyaan-create" class="btn btn-add-pertanyaan">
        ...
        <div class="card-body">
            ...
            <?php if (empty($pertanyaans)): ?>
                <div class="empty-state">
                    ...
                    <p>Mulai buat pertanyaan untuk survei Anda</p>
                    <a href="/webqual/admin/pertanyaan-create" class="btn btn-add-pertanyaan">
                        <i class="bi bi-plus-circle"></i>
            ...
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-modern">
                        ...
                        <tbody>
                            <?php $no = 1; foreach ($pertanyaans as $p): ?>
                                <tr>
                                    <td>
                                        <span class="pertanyaan-id"><?= $no++; ?></span>
                                    </td>
                                    <td>
                                        <span class="pertanyaan-code"><?= htmlspecialchars($p['kode_pertanyaan']); ?></span>
                                    </td>
                                    <td>
                                        <div class="pertanyaan-text">
                                            <?= htmlspecialchars(mb_strimwidth($p['teks_pertanyaan'], 0, 90, '...')); ?>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="dimensi-name">
                                            <i class="bi bi-diagram-3"></i>
                                            <span><?= htmlspecialchars($p['nama_dimensi'] ?? ($mapDim[(int)$p['id_dimensi']] ?? '-')); ?></span>
                                        </div>
                                    </td>
                                    <td>
                                        <?= badgeAktif($p['aktif']); ?>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="/webqual/admin/pertanyaan-show?id_pertanyaan=<?= $p['id_pertanyaan'] ?>" 
                                               class="btn btn-action btn-view" 
                                               title="Lihat Detail">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            <a href="/webqual/admin/pertanyaan-edit?id_pertanyaan=<?= $p['id_pertanyaan'] ?>" 
                                               class="btn btn-action btn-edit"
                                               title="Edit">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <button onclick="deletePertanyaan('<?= (int)$p['id_pertanyaan'] ?>')" 
                ...
                <div class="pagination-wrapper">
                    <div class="pagination-info">
                        Menampilkan <strong id="showingStart">1</strong> - <strong id="showingEnd">5</strong> dari <strong id="totalData"><?= count($pertanyaans); ?></strong> data
                    </div>
...
<script>
    ...
    function showPage(page) {
        ...
        pageRows.forEach((row, index) => {
            const numberCell = row.querySelector('.pertanyaan-id');
            if (numberCell) {
    ...
    function deletePertanyaan(id) {
        if (confirm('Yakin ingin menghapus pertanyaan ini?\n\nPastikan pertanyaan tidak sedang dipakai di survei aktif.')) {
            // Show loading state
            ...
            $.post('/webqual/admin/pertanyaan/delete', {
                ...
                if (j.status === 'success') {
                    ...
                    alert('✗ Gagal menghapus pertanyaan.\nMungkin sedang dipakai di survei aktif.');
